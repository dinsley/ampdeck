...
type Phase =
	"idle" | "working" | "thinking" | "researching" | "editing" | "testing" | "shipping" | "error" | "done" | "cancelled";
...
export default function streamDeckCompanion(amp: PluginAPI) {
	...
	const executorKind =
		amp.system.executor.kind === "local" || amp.system.executor.kind === "remote"
			? amp.system.executor.kind
			: "unknown";
	...
	async function connect(): Promise<void> {
		...
		const authenticationTimer = setTimeout(
			() => candidate.close(4001, "Authentication timeout"),
			...
			result = (async (): Promise<CommandResult> => {
				...
				try {
					if (command.command === "acknowledge") {
						...
						await thread.appendUserMessage({ type: "user-message", content: command.content! });
	...
	amp.activeThread.subscribe((thread) => {
		if (thread)
			void watchThread(thread.id).then((watchedThread) => {
				watchedThread.unread = false;
				emitStatus(watchedThread);
			});
	});
	...
	amp.on("session.start", (event) => {
		void watchThread(event.thread.id);
	});
	amp.on("agent.start", (event) => {
		setPhase(event.thread.id, "thinking", "running");
	});
	amp.on("tool.result", (event) => {
		setPhase(
			event.thread.id,
			event.status === "error" ? "error" : classifyTool(event.tool),
...
function parseBridgeMessage(value: unknown): Challenge | Acknowledgement | ThreadCommand | undefined {
	try {
		...
		if (message.type === "hello.challenge")
			return isHex(message.clientNonce, 64) && isHex(message.serverNonce, 64) && isHex(message.proof, 64)
				? (message as Challenge)
				: undefined;
		if (message.type === "hello.ack")
			return isHex(message.serverNonce, 64) && isHex(message.proof, 64) ? (message as Acknowledgement) : undefined;
		if (
			message.type !== "thread.command" ||
			typeof message.commandID !== "string" ||
			message.commandID.length === 0 ||
			message.commandID.length > 64 ||
			...
			(message.content !== undefined && (typeof message.content !== "string" || message.content.length > 4096))
		)
			return undefined;
...
async function proof(
	...
	const signature = await crypto.subtle.sign(
		"HMAC",
		key,
		encoder.encode(`${role}|${clientId}|${clientNonce}|${serverNonce}`),
	);
